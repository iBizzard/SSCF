...
class RandomRotate:
    def __init__(self, random_state, angle_spectrum=10, axes=None, mode='reflect', order=0, **kwargs):
        if axes is None:
            axes = [(0, 1)]  # Only rotate around the height-width axis
        else:
    ...
    def __call__(self, volumes, label, type):
        axis = self.axes[0]  # Since only one axis is defined, use it directly
        ...
        for i in range(len(volumes) + 1):
            ...
            if m.ndim == 2:
                ...
                raise ValueError("This function only supports 2D images.")
            if i != len(volumes):
                volumes[i] = m
            else:
                label = m
...
class RandomFlip:
    ...
    def __call__(self, volumes, label, type):
        ...
        assert label.ndim == 2, 'Supports only 2D (HxW) images'
...
class ToLongTensor:
    ...
    def __call__(self, lbl):
        ...
        return torch.from_numpy(lbl).float()
